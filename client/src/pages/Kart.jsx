...
const total = subTotal; // For now, total is same as subtotal
  return (
    <div className="py-8 relative flex flex-col items-center min-h-[50vh]">
      ...
      {cartItems.map(item => (
            item.productDetails ? (
      <div key={item.productId}  className="flex border rounded-xl w-full overflow-hidden relative bg-white ">
      <img className="w-[40%] md:w-[20%]" src={item.productDetails.photos} alt="" />
      <div className="flex flex-col p-2">
        <h1 className="font-bold cursor-pointer hover:underline">{item.productDetails.companymodel
        }</h1>
        <h2 className="font-medium ">$ {item.productDetails.price}</h2>
        <h3 className="font-thin mt-4 text-xs">Added On : 31st Feb 2024</h3>
        <div className="flex absolute right-2 bottom-2 bg-black text-white rounded px-2 py-1 invert cursor-pointer"   onClick={() => handleDelete(item.productId)}>
      ...
      <div className="flex justify-center">
        <button className="border border-black rounded p-2 " onClick={()=>navigate("/billingPage")}>
          Proceed to payment
        </button>
      </div>
    </div>
